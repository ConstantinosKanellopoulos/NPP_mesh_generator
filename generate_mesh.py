...
embedment = 0 # if embedment is activated, the last two soil layers with thickness 4.5 and 7 (see right below in SL) will be generated (note: metamaterials can be generated only if embedment is activated, so if metatamaterials are activated, the last 2 soil layers will be generated automatically)
...
separation = 0 # 0 means gap, 1 means filled with soil (note: it will make a difference only when embedment is activated (i.e., the last 2 soil layers of thickness 4.5 and 7 in SL are generated))

# number of DRM layers (6 are used in the paper)
DRM_l = 6 # creating DRM layers takes time, try with 0 or 1 DRM layers first if you just want to visualize quickly the parts of the model
